...
public class BlockTorchUnlit extends BlockTorch {
    ...
    public ItemStack getPickBlock(IBlockState state, @Nullable RayTraceResult target,
        ...
        return ARConfiguration.getCurrentConfig().dropExTorches ?
                super.getPickBlock(state, target, world, pos, player) : new ItemStack(Blocks.TORCH);
    }
    ...
    public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos,
        ...
        ret.add(new ItemStack(ARConfiguration.getCurrentConfig().dropExTorches ?
                AdvancedRocketryBlocks.blockUnlitTorch : Blocks.TORCH));
    ...
    public boolean onBlockActivated(@Nonnull World world, @Nonnull BlockPos pos,
                                    @Nonnull IBlockState state, @Nonnull EntityPlayer player, EnumHand hand,
        ...
        if (!player.getHeldItem(EnumHand.MAIN_HAND).isEmpty()) {
            ...
            if (atmhandler != null && !world.isRemote && !item.equals(Items.AIR) &&
                    atmhandler.getAtmosphereType(pos).allowsCombustion() &&
                    (item == Item.getItemFromBlock(Blocks.TORCH) || item == Items.FLINT_AND_STEEL ||
