...
public class PacketFluidParticle extends BasePacket {
    ...
    public void executeClient(EntityPlayer thePlayer) {
        if (Minecraft.getMinecraft().gameSettings.particleSetting < 1) {
            InverseTrailFluid fx3 = new InverseTrailFluid(thePlayer.world, fromPos.getX() + 0.5, fromPos.getY() + 0.5,
                    fromPos.getZ() + 0.5, toPos.getX() + 0.5, toPos.getY() + 0.5, toPos.getZ() + 0.5, color, time);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx3);
        }

        int numIterations = Minecraft.getMinecraft().gameSettings.particleSetting < 1 ? 5 :
                (Minecraft.getMinecraft().gameSettings.particleSetting < 2 ? 3 : 0);

        for (int i = 0; i < numIterations; i++) {
            InverseTrailFluid fx2 = new InverseTrailFluid(thePlayer.world, fromPos.getX() + 0.1 * (0.5 - Math.random()),
                    fromPos.getY() + 0.1 * (0.5 - Math.random()), fromPos.getZ() + 0.1 * (0.5 - Math.random()),
                    toPos.getX() + 0.5, toPos.getY() + 0.5, toPos.getZ() + 0.5, color, time);
