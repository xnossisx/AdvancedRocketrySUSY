...
public class ClientHelper {
     ...
     * }
     * 
     * public static void netHandlerSetPlayerLocation(EntityPlayerMP player,double a, double b, double c, float d, float
     * e, int rotation) {
     * if(rotation == 0)
     * player.playerNetServerHandler.sendPacket(new S08PacketPlayerPosLook(a, b + 1.6200000047683716D, c, d, e, false));
     * else
     * player.playerNetServerHandler.sendPacket(new S08PacketPlayerPosLook(player.posX, player.posY + player.width/2f,
     * player.posZ, d, e, false));
     ...
     * double d9;
     * 
     * for (d9 = 0.05D; x != 0.0D && entity.worldObj.getCollidingBoundingBoxes(entity,
     * entity.boundingBox.getOffsetBoundingBox(x, -1.0D, 0.0D)).isEmpty(); d6 = x)
     * {
     * if (x < d9 && x >= -d9)
     * {
     * x = 0.0D;
     ...
     * }
     * 
     * for (; z != 0.0D && entity.worldObj.getCollidingBoundingBoxes(entity,
     * entity.boundingBox.getOffsetBoundingBox(0.0D, -1.0D, z)).isEmpty(); d8 = z)
     * {
     * if (z < d9 && z >= -d9)
     * {
     * z = 0.0D;
     ...
     * }
     * 
     * while (x != 0.0D && z != 0.0D && entity.worldObj.getCollidingBoundingBoxes(entity,
     * entity.boundingBox.getOffsetBoundingBox(x, -1.0D, z)).isEmpty())
     * {
     * if (x < d9 && x >= -d9)
     * {
     * x = 0.0D;
     ...
     * entity.distanceWalkedOnStepModified = (float)((double)entity.distanceWalkedOnStepModified +
     ...
     * if (entity.isInWater())
     * {
     * float f = MathHelper.sqrt_double(entity.motionX * entity.motionX * 0.20000000298023224D + entity.motionY *
     * entity.motionY + entity.motionZ * entity.motionZ * 0.20000000298023224D) * 0.35F;
     * 
     * if (f > 1.0F)
     * {
     * f = 1.0F;
     ...
     * }
     * 
     * public static void transformCamera2(int rotation, EntityLivingBase base/*float yaw, float pitch, float prevYaw,
     * float prevPitch* /,float p_78467_1_) {
     * 
     * //Debug code
     * if(rotation == 0)
     * return;
     ...
     * 
     * if(rotation == 1) {
     * GL11.glRotatef(base.rotationPitch + (base.rotationPitch - base.prevRotationPitch) * p_78467_1_, 0.0F, -1.0F,
     * 0.0F);
     * GL11.glRotatef(base.rotationYaw + (base.rotationYaw - base.prevRotationYaw) * p_78467_1_ + 180.0F, 1.0F, 0.0F,
     * 0.0F);
     * GL11.glRotatef(90, 0f, 1f, 0f);
     * }
     * if(rotation == 2) {
