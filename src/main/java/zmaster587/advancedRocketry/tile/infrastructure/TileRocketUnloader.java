...
public class TileRocketUnloader extends TileRocketLoader
                            ...
                            for (int j = 0; j < getSizeInventory(); j++) {
                                if (getStackInSlot(j).isEmpty()) {
                                    ...
                                    break out;
                                        ...
                                        isItemValidForSlot(j, inv.getStackInSlot(i))) {
                                            if (isAllowedToOperate) {
                                                ItemStack stack2 = inv.decrStackSize(i,
                                                        getStackInSlot(j).getMaxStackSize() -
                                                                getStackInSlot(j).getCount());
                                                getStackInSlot(j)
                                                        .setCount(getStackInSlot(j).getCount() + stack2.getCount());
                                            }
                                            if (inv.getStackInSlot(i).isEmpty())
                                                break out;
                                            foundStack = true;
