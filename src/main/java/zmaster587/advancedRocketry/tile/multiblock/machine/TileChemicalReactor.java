...
public class TileChemicalReactor extends TileMultiblockMachine {
    ...
    public static void registerRecipe(RecipesMachine recipesMachine, Item item) {
        if (item instanceof ItemArmor && !(item instanceof ItemSpaceArmor)) {
            ...
            if (((ItemArmor) item).armorType == EntityEquipmentSlot.CHEST)
                recipesMachine.addRecipe(TileChemicalReactor.class, enchanted, 100, 10,
                        new ItemStack(item, 1, OreDictionary.WILDCARD_VALUE),
                        new ItemStack(AdvancedRocketryBlocks.blockPipeSealer, 1),
                        new NumberedOreDictStack("sheetTitaniumAluminide", 4),
                        new ItemStack(AdvancedRocketryItems.itemPressureTank, 1, 3));
            else
                recipesMachine.addRecipe(TileChemicalReactor.class, enchanted, 100, 10,
                        new ItemStack(item, 1, OreDictionary.WILDCARD_VALUE),
                        new ItemStack(AdvancedRocketryBlocks.blockPipeSealer, 1),
    ...
    public void onInventoryUpdated() {
        ...
        if (getOutputs() == null && (recipe = getRecipe(getMachineRecipeList())) != null && canProcessRecipe(recipe) &&
                ...
                EnchantmentHelper.getEnchantmentLevel(AdvancedRocketryAPI.enchantmentSpaceProtection,
                        recipe.getOutput().get(0)) == 1) {
            if (!enabled) {
                        ...
                        if (!stackInSlot.isEmpty() && stackInSlot.getCount() >= stack.getCount() &&
                            ...
                            if (stack2.getItem() instanceof ItemArmor) {
                                if (EnchantmentHelper.getEnchantmentLevel(
                                        AdvancedRocketryAPI.enchantmentSpaceProtection, stack2) == 0) {
                            ...
                            hatch.markDirty();
                            world.notifyBlockUpdate(pos, world.getBlockState(((TileEntity) hatch).getPos()),
                                    world.getBlockState(((TileEntity) hatch).getPos()), 6);
