...
public class TileBiomeScanner extends TileMultiPowerConsumer {
    ...
    private static final Object[][][] structure = new Object[][][] {
            ...
            { { null, "blockAluminum", "blockAluminum", "blockAluminum", null },
                    { "blockAluminum", "blockAluminum", AdvancedRocketryBlocks.blockStructureTower, "blockAluminum",
                            "blockAluminum" },
                    { "blockAluminum", AdvancedRocketryBlocks.blockStructureTower, LibVulpesBlocks.blockStructureBlock,
                            AdvancedRocketryBlocks.blockStructureTower, "blockAluminum" },
                    { "blockAluminum", "blockAluminum", AdvancedRocketryBlocks.blockStructureTower, "blockAluminum",
                            "blockAluminum" },
                    { null, "blockAluminum", "blockAluminum", "blockAluminum", null } },
        ...
        if (world.isRemote) {
            ...
            if (suitable && SpaceObjectManager.WARPDIMID != spaceObject.getOrbitingPlanetId()) {
                ...
                DimensionProperties properties = DimensionManager.getInstance()
                        .getDimensionProperties(spaceObject.getOrbitingPlanetId());
                List<ModuleBase> list2 = new LinkedList<>();
                if (properties.isGasGiant()) {
                    list2.add(new ModuleText(32, 16, LibVulpes.proxy.getLocalizedString("msg.biomescanner.gas"),
                            0x202020));
                } else if (properties.isStar()) {
                    list2.add(new ModuleText(32, 16, LibVulpes.proxy.getLocalizedString("msg.biomescanner.star"),
                            0x202020));
                } else {
                    ...
                    if (properties.getId() == 0) {
                        for (Biome biome : Biome.REGISTRY) {
                            if (biome != null)
                                list2.add(new ModuleText(32, 16 + 12 * (i++),
                                        AdvancedRocketry.proxy.getNameFromBiome(biome), 0x202020));
                        }
                    } else {
                        for (BiomeEntry biome : properties.getBiomes()) {
                            list2.add(new ModuleText(32, 16 + 12 * (i++),
                                    AdvancedRocketry.proxy.getNameFromBiome(biome.biome), 0x202020));
                        }
                    }
                }
                // Relying on a bug, is this safe?
                ModuleContainerPan pan = new ModuleContainerPan(0, 16, list2, new LinkedList<>(), null, 148, 110, 0,
                        -64, 0, 1000);
                list.add(pan);
            } else
