...
class VoidDrill extends AbstractDrill {
                ...
                if (globalOres != null && !globalOres.isEmpty()) {
                    ...
                    try {
                        ...
                        AdvancedRocketry.logger
                                .warn("Unexpected ore name: \"" + oreDictName + "\" during laser drill harvesting");
                        continue;
                    }
    ...
    boolean activate(World world, int x, int z) {
        // Ideally, this should be done in the constructor, but the world provider is null there for reasons unknown, so
        // this gets delayed until first activation
        this.ores = null;
        this.planetOresInitialized = false;
        loadGlobalOres();
        DimensionProperties dimProperties = DimensionManager.getInstance()
