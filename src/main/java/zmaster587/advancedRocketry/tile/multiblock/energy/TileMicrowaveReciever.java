...
public class TileMicrowaveReciever extends TileMultiPowerProducer implements ITickable {
    ...
    public void update() {
        ...
        // Checks whenever a station changes dimensions or when the multiblock is intialized - ie any time the multipler
        // could concieveably change
        if (insolationPowerMultiplier == 0 ||
                ((world.provider.getDimension() == ARConfiguration.getCurrentConfig().spaceDimId) &&
                        (powerSourceDimensionID != SpaceObjectManager.getSpaceManager()
                                .getSpaceStationFromBlockCoords(this.pos).getOrbitingPlanetId()))) {
            DimensionProperties properties = DimensionManager.getInstance()
                    .getDimensionProperties(world.provider.getDimension());
            insolationPowerMultiplier = (world.provider.getDimension() ==
            ...
            if (world.provider.getDimension() == ARConfiguration.getCurrentConfig().spaceDimId)
                powerSourceDimensionID = SpaceObjectManager.getSpaceManager().getSpaceStationFromBlockCoords(this.pos)
                        .getOrbitingPlanetId();
        }
        if (!isComplete())
            return;
        ...
        if (!world.isRemote && getPowerMadeLastTick() > 0 && world.getTotalWorldTime() % 100 == 0) {
            ...
            List<Entity> entityList = world.getEntitiesWithinAABB(Entity.class,
                    new AxisAlignedBB(this.getPos().getX() - offset.x, this.getPos().getY(),
                            this.getPos().getZ() - offset.z,
                            this.getPos().getX() - offset.x + getStructure()[0][0].length, 256,
                            this.getPos().getZ() - offset.z + getStructure()[0].length));
            ...
            for (int x = 0; x < getStructure()[0][0].length; x++) {
                for (int z = 0; z < getStructure()[0].length; z++) {
                    ...
                    IBlockState state = world.getBlockState(
                            pos2 = (world.getHeight(pos.add(x - offset.x, 128, z - offset.z)).add(0, -1, 0)));

                    if (pos2.getY() > this.getPos().getY()) {
                        if (!world.isAirBlock(pos2.add(0, 1, 0))) {
                            world.setBlockToAir(pos2);
                            world.playSound(pos2.getX(), pos2.getY(), pos2.getZ(),
                                    new SoundEvent(new ResourceLocation("fire.fire")), SoundCategory.BLOCKS, 1f, 3f,
                                    false);
                        }
                    }
                }
        ...
        SpaceStationObject spaceStation = (SpaceStationObject) SpaceObjectManager.getSpaceManager()
                .getSpaceStationFromBlockCoords(this.pos);
        if (!world.isRemote &&
                (DimensionManager.getInstance().isDimensionCreated(dimId) || world.provider.getDimension() == 0)) {
            // This way we check to see if it's on a station, and if so, if it has any satellites in orbit around the
            // planet the station is around to pull from
            properties = (spaceStation != null) ? spaceStation.getOrbitingPlanet() :
            ...
            if (enabled) {
                for (long lng : connectedSatellites) {
                    ...
                    if (satellite instanceof IUniversalEnergyTransmitter) {
                        energyReceived += ((IUniversalEnergyTransmitter) satellite).transmitEnergy(EnumFacing.UP,
                                false);
                    }
                }
