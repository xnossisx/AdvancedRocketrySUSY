...
public class TileSatelliteBuilder extends TileMultiPowerConsumer
                                  implements IModularInventory, IInventory, IButtonInventory {
    ...
    public boolean canAssembleSatellite() {
        ...
        for (int i = primaryFunctionSlot; i <= modularFunctionSlotEnd; i++) {
            ...
            if (!stack.isEmpty() && SatelliteRegistry.getSatelliteProperty(stack) != null &&
                    SatelliteRegistry.getSatelliteProperty(stack).getPropertyFlag() ==
                            SatelliteProperties.Property.POWER_GEN.getFlag() &&
                    SatelliteRegistry.getSatelliteProperty(stack).getPowerGeneration() > 0)
                hasPowerGeneration = true;
        }

        // Make sure critical parts exist and output is empty
        if (getStackInSlot(primaryFunctionSlot).isEmpty() ||
                SatelliteRegistry.getSatelliteProperty(getStackInSlot(primaryFunctionSlot)) == null ||
                !hasPowerGeneration)
            return false;
    ...
    public void assembleSatellite() {
        ...
        if (!world.isRemote) {
            ...
            for (int currentSlotIndex = modularFunctionSlotStart; currentSlotIndex <=
                    modularFunctionSlotEnd; currentSlotIndex++) {
                ItemStack stack = getStackInSlot(currentSlotIndex);
                if (SatelliteRegistry.getSatelliteProperty(stack) != null) {
                    if (SatelliteRegistry.getSatelliteProperty(stack).getPropertyFlag() ==
                            SatelliteProperties.Property.BATTERY.getFlag())
                        powerStorage += SatelliteRegistry.getSatelliteProperty(getStackInSlot(currentSlotIndex))
                                .getPowerStorage();
                    if (SatelliteRegistry.getSatelliteProperty(stack).getPropertyFlag() ==
                            SatelliteProperties.Property.POWER_GEN.getFlag())
                        powerGeneration += SatelliteRegistry.getSatelliteProperty(getStackInSlot(currentSlotIndex))
                                .getPowerGeneration();
                    if (SatelliteRegistry.getSatelliteProperty(stack).getPropertyFlag() ==
                            SatelliteProperties.Property.DATA.getFlag())
                        maxData += SatelliteRegistry.getSatelliteProperty(getStackInSlot(currentSlotIndex))
                                .getMaxDataStorage();
                }
            ...
            SatelliteProperties properties = new SatelliteProperties(powerGeneration, powerStorage + 720, satType,
                    maxData, weight);
    ...
    private boolean canCopy() {
        ...
        if (!chipsExist)
            return false;
        boolean isSatellite = ((stack0.getItem() instanceof ItemSatellite ||
                stack0.getItem() instanceof ItemSatelliteIdentificationChip) &&
                stack1.getItem().equals(stack0.getItem()));
        boolean isStation = stack0.getItem() instanceof ItemStationChip && ItemStationChip.getUUID(stack0) != 0 &&
                stack1.getItem() instanceof ItemStationChip;
        boolean isPlanet = (stack0.getItem() instanceof ItemPlanetIdentificationChip &&
                stack1.getItem() instanceof ItemPlanetIdentificationChip);
        boolean isOreScanner = (stack0.getItem() instanceof ItemOreScanner &&
                stack1.getItem() instanceof ItemOreScanner);
    ...
    private void copyChip() {
        ...
        if (slot0.getItem() instanceof ItemSatelliteIdentificationChip || slot0.getItem() instanceof ItemOreScanner ||
                slot0.getItem() instanceof ItemPlanetIdentificationChip || slot0.getItem() instanceof ItemStationChip) {
    ...
    public List<ModuleBase> getModules(int ID, EntityPlayer player) {
        ...
        modules.add(new ModuleTexturedLimitedSlotArray(152, 10, this, 0, 1, TextureResources.functionComponent)); // Functional
                                                                                                                  // Piece
        ...
        modules.add(new ModuleTexturedLimitedSlotArray(38, 16, this, chassisSlot, chassisSlot + 1,
                TextureResources.slotSatellite));    // Id chip
        modules.add(new ModuleOutputSlotArray(58, 36, this, 7, 8));   // Output
        modules.add(new ModuleTexturedSlotArray(58, 16, this, chipSlot, chipSlot + 1, TextureResources.idChip));    // Id
                                                                                                                    // chip
        modules.add(new ModuleTexturedSlotArray(82, 16, this, chipCopySlot, chipCopySlot + 1, TextureResources.idChip));    // Id
                                                                                                                            // chip
        modules.add(new ModuleProgress(75, 36, 0,
                new ProgressBarImage(217, 0, 17, 17, 234, 0, EnumFacing.DOWN, TextureResources.progressBars), this));
        modules.add(new ModuleButton(40, 56, 0, "Build", this,
                zmaster587.libVulpes.inventory.TextureResources.buttonBuild));
        modules.add(new ModuleButton(173, 3, 1, "", this, TextureResources.buttonCopy,
                LibVulpes.proxy.getLocalizedString("msg.satbuilder.writesecondchip"), 24, 24));
