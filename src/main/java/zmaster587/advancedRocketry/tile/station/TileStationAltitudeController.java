...
public class TileStationAltitudeController extends TileEntity implements IModularInventory, ITickable, INetworkMachine,
                                           ISliderBar, IButtonInventory, IComparatorOverride {
    ...
    public TileStationAltitudeController() {
        ...
        // maxGravBuildSpeed = new ModuleText(6, 25,
        // LibVulpes.proxy.getLocalizedString("msg.stationaltctrl.maxaltrate"), 0xaa2020);
    ...
    private void updateText() {
        if (world.isRemote) {
            ...
            if (spaceObject != null) {
                moduleGrav.setText(
                        String.format("%s %.0fKm", LibVulpes.proxy.getLocalizedString("msg.stationaltctrl.alt"),
                                spaceObject.getOrbitalDistance() * 200 + 100));
                // maxGravBuildSpeed.setText(String.format("%s%.1f",
                // LibVulpes.proxy.getLocalizedString("msg.stationaltctrl.maxaltrate"), 7200D *
                // spaceObject.getMaxRotationalAcceleration()));
                targetGrav
                        .setText(String.format("%s %d", LibVulpes.proxy.getLocalizedString("msg.stationaltctrl.tgtalt"),
                                ((SpaceStationObject) spaceObject).targetOrbitalDistance * 200 + 100));
            }
            ...
            if (!world.isRemote) {
                ...
                if (spaceObject != null) {
                    if (redstoneControl.getState() == RedstoneState.ON)
                        ((SpaceStationObject) spaceObject).targetOrbitalDistance = Math
                                .max((world.getStrongPower(pos) * 13) + 4, 190);
                    else if (redstoneControl.getState() == RedstoneState.INVERTED)
                        ((SpaceStationObject) spaceObject).targetOrbitalDistance = Math
                                .max(Math.abs(15 - world.getStrongPower(pos)) * 13 + 4, 190);
                    ...
                    if (difference != 0) {
                        ...
                        if (!world.isRemote) {
                            ...
                            // PacketStationUpdate.Type.ROTANGLE_UPDATE), this.worldObj.provider.dimensionId,
                            // this.xCoord, this.yCoord, this.zCoord, 1024);
                            PacketHandler.sendToAll(
                                    new PacketStationUpdate(spaceObject, PacketStationUpdate.Type.ALTITUDE_UPDATE));
                            markDirty();
                        } else
                            updateText();
                    }
    ...
    public void setProgress(int id, int progress) {
        ...
        if (SpaceObjectManager.getSpaceManager().getSpaceStationFromBlockCoords(this.pos) != null) {
            ((SpaceStationObject) (SpaceObjectManager.getSpaceManager()
                    .getSpaceStationFromBlockCoords(this.pos))).targetOrbitalDistance = progress;
        }
    }
