...
public class TileStationOrientationController extends TileEntity implements ITickable, IModularInventory,
                                              INetworkMachine, ISliderBar, IButtonInventory {
    ...
    public TileStationOrientationController() {
        moduleAngularVelocity = new ModuleText(6, 15, LibVulpes.proxy.getLocalizedString("msg.stationorientctrl.alt"),
                0xaa2020);
        // numThrusters = new ModuleText(10, 25, "Number Of Thrusters: ", 0xaa2020);
        targetRotations = new ModuleText(6, 25, LibVulpes.proxy.getLocalizedString("msg.stationorientctrl.tgtalt"),
                0x202020);
        progress = new int[3];
    ...
    public List<ModuleBase> getModules(int id, EntityPlayer player) {
        ...
        modules.add(new ModuleButton(25, 35, 2, LibVulpes.proxy.getLocalizedString("msg.spacelaser.reset"), this,
                zmaster587.libVulpes.inventory.TextureResources.buttonBuild, 36, 15));
    ...
    private void updateText() {
        if (world.isRemote) {
            ...
            if (spaceObject != null) {
                moduleAngularVelocity.setText(String.format("%s%.1f %.1f %.1f",
                        LibVulpes.proxy.getLocalizedString("msg.stationorientctrl.alt"),
                        72000D * spaceObject.getDeltaRotation(EnumFacing.EAST),
                        72000D * spaceObject.getDeltaRotation(EnumFacing.UP),
                        7200D * spaceObject.getDeltaRotation(EnumFacing.NORTH)));
                // maxAngularAcceleration.setText(String.format("Maximum Angular Acceleration: %.1f",
                // 7200D*object.getMaxRotationalAcceleration()));
                ...
                targetRotations.setText(
                        String.format("%s%d %d %d", LibVulpes.proxy.getLocalizedString("msg.stationorientctrl.tgtalt"),
                                targetRotationsPerHour[0], targetRotationsPerHour[1], targetRotationsPerHour[2]));
            }
        }
    }
            ...
            if (!world.isRemote) {
                ...
                if (spaceObject != null) {
                    ...
                    if (!world.isRemote && update) {
                        // PacketHandler.sendToNearby(new PacketStationUpdate(spaceObject,
                        // PacketStationUpdate.Type.ROTANGLE_UPDATE), this.worldObj.provider.dimensionId, this.xCoord,
                        // this.yCoord, this.zCoord, 1024);
                        PacketHandler.sendToAll(
                                new PacketStationUpdate(spaceObject, PacketStationUpdate.Type.ROTANGLE_UPDATE));
                    }
                } else
                    updateText();
            } else
    ...
    public void setProgress(int id, int progress) {
        ...
        if (SpaceObjectManager.getSpaceManager().getSpaceStationFromBlockCoords(this.pos) != null) {
            ((SpaceStationObject) (SpaceObjectManager.getSpaceManager().getSpaceStationFromBlockCoords(this.pos)))
                    .setTargetRotationsPerHour(id, (progress - getTotalProgress(id) / 2));
        }
    }
