...
public class TileStationGravityController extends TileEntity implements IModularInventory, ITickable, INetworkMachine,
    ...
    public TileStationGravityController() {
        ...
        maxGravBuildSpeed = new ModuleText(6, 25, LibVulpes.proxy.getLocalizedString("msg.stationgravctrl.maxaltrate"),
                0xaa2020);
    ...
    private void updateText() {
        if (world.isRemote) {
            ...
            if (spaceObject != null) {
                moduleGrav
                        .setText(String.format("%s%.2f", LibVulpes.proxy.getLocalizedString("msg.stationgravctrl.alt"),
                                spaceObject.getProperties().getGravitationalMultiplier()));
                maxGravBuildSpeed.setText(
                        String.format("%s%.1f", LibVulpes.proxy.getLocalizedString("msg.stationgravctrl.maxaltrate"),
                                7200D * spaceObject.getMaxRotationalAcceleration()));
                targetGrav
                        .setText(String.format("%s%d", LibVulpes.proxy.getLocalizedString("msg.stationgravctrl.tgtalt"),
            ...
            if (!world.isRemote) {
                ...
                if (spaceObject != null) {
                    if (redstoneControl.getState() == RedstoneState.ON)
                        ...
                        ((SpaceStationObject) spaceObject).targetGravity = Math.abs(15 - world.getStrongPower(pos)) *
                                6 + 10;
                    ...
                    if (Math.abs(difference) >= 0.001) {
                        ...
                        if (!world.isRemote) {
                            ...
                            PacketHandler.sendToAll(
                                    new PacketStationUpdate(spaceObject, PacketStationUpdate.Type.DIM_PROPERTY_UPDATE));
                            markDirty();
                        } else
                            updateText();
                    }
    ...
    public void setProgress(int id, int progress) {
        ...
        if (SpaceObjectManager.getSpaceManager().getSpaceStationFromBlockCoords(this.pos) != null) {
            ((SpaceStationObject) (SpaceObjectManager.getSpaceManager()
                    .getSpaceStationFromBlockCoords(this.pos))).targetGravity = progress + minGravity;
        }
    }
    ...
    public int getComparatorOverride() {
        if (this.world.provider instanceof WorldProviderSpace) {
            if (!world.isRemote) {
                ISpaceObject spaceObject = SpaceObjectManager.getSpaceManager().getSpaceStationFromBlockCoords(pos);
                if (spaceObject != null) {
                    return (int) ((((SpaceStationObject) spaceObject).getProperties().getGravitationalMultiplier() -
                            0.1) / 0.059);
                }
            }
        }
