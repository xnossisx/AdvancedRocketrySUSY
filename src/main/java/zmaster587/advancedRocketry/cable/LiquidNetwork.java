...
public class LiquidNetwork extends CableNetwork {
    ...
    public void tick() {
        ...
        while (sinkItr.hasNext()) {
            ...
            IFluidHandler fluidHandleSink = obj.getKey().getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY,
                    obj.getValue());
            ...
            if (fluidHandleSink == null) {
                sinkItr.remove();
                AdvancedRocketry.logger.info("Tile at " + obj.getKey().getPos() +
                        " is added as a sink but has no fluid capabilities on the side connected");
                continue;
            }
            ...
            if (fluid == null) {
                ...
                while (sourceItr.hasNext()) {
                    ...
                    IFluidHandler fluidHandleSource = objSource.getKey()
                            .getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, obj.getValue());

                    if (fluidHandleSource == null) {
                        sourceItr.remove();
                        AdvancedRocketry.logger.info("Tile at " + obj.getKey().getPos() +
                                " is added as a source but has no fluid capabilities on the side connected");
                        continue;
                    }
