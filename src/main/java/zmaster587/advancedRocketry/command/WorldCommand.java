...
public class WorldCommand implements ICommand {
    ...
    private void commandAddTorch(ICommandSender sender, String[] cmdstring) {
        if (cmdstring.length >= 2 && cmdstring[1].equalsIgnoreCase("help")) {
            sender.sendMessage(new TextComponentString(aliases.get(0) + " " + cmdstring[0] +
                    " - Adds the currently held block to the list of objects that drop when there's no atmosphere"));
            return;
        }
    ...
    private void commandAddSolidBlockOverride(ICommandSender sender, String[] cmdstring) {
        if (cmdstring.length >= 2 && cmdstring[1].equalsIgnoreCase("help")) {
            sender.sendMessage(new TextComponentString(aliases.get(0) + " " + cmdstring[0] +
                    " - Adds the currently held block to the list of blocks that can hold a seal"));
            return;
        }
        ...
        if (block != Blocks.AIR) {
            if (ARConfiguration.getCurrentConfig().torchBlocks.contains(block))
                sender.sendMessage(
                        new TextComponentString(block.getLocalizedName() + " is already in the sealed blocks list"));
            else {

                ARConfiguration.getCurrentConfig().addSealedBlock(block);

                sender.sendMessage(
                        new TextComponentString(block.getLocalizedName() + " added to the sealed block list"));
            }
        } else
    ...
    private void commandGiveStation(ICommandSender sender, String[] cmdstring) {
        if (cmdstring.length < 2 || cmdstring[1].equalsIgnoreCase("help")) {
            sender.sendMessage(new TextComponentString(aliases.get(0) + " " + cmdstring[0] +
                    " - Gives the player playerName (if supplied) a spacestation with ID stationID"));
            sender.sendMessage(
                    new TextComponentString("Usage: /advrocketry " + cmdstring[0] + " <stationId> [PlayerName]"));
            return;
        }
    ...
    private void commandFillData(ICommandSender sender, String[] cmdstring) {
        ...
        if (sender.getCommandSenderEntity() != null) {
            ...
            if (cmdstring[1].equalsIgnoreCase("help")) {
                sender.sendMessage(
                        new TextComponentString(aliases.get(0) + " " + cmdstring[0] + " [datatype] [amountFill]\n"));
                sender.sendMessage(new TextComponentString(
                        "Fills the amount of the data type specifies into the chip being held."));
                sender.sendMessage(new TextComponentString(
                        "If the datatype is not specified then command fills all datatypes, if no amountFill is specified completely fills the chip"));
                return;
            }
            ...
            if (!stack.isEmpty() && stack.getItem() instanceof ItemData) {
                ...
                try {
                    ...
                    sender.sendMessage(new TextComponentString(
                            "Did you mean: /advrocketry" + cmdstring[0] + " [datatype] [amountFill]"));
    ...
    private void commandReloadRecipes(ICommandSender sender, String[] cmdstring) {
        if (cmdstring.length >= 2 && cmdstring[1].equalsIgnoreCase("help")) {
            sender.sendMessage(new TextComponentString(aliases.get(0) + " " + cmdstring[0] +
                    " - Reloads recipes from the XML files in the config folder"));
            return;
        }
        ...
        try {
            ...
            sender.sendMessage(new TextComponentString(
                    "You may be able to rectify this error by repairing the XML and/or restarting the game"));
        }
    }
    ...
    private void commandSetGravity(ICommandSender sender, String[] cmdstring) {
        if (cmdstring.length >= 2) {
            if (cmdstring[1].equalsIgnoreCase("help")) {
                sender.sendMessage(new TextComponentString(
                        cmdstring[0] + " <amount> - sets your gravity to amount where 1 is Earth-like"));
                return;
            }
            ...
            sender.sendMessage(
                    new TextComponentString(aliases.get(0) + " " + cmdstring[0] + " gravity_multiplier [playerName]"));
            sender.sendMessage(new TextComponentString(""));
            sender.sendMessage(new TextComponentString(
                    "Use 0 as the gravity_multiplier to allow regular planet gravity to take over"));
        }
    }
    ...
    private void commandGoto(ICommandSender sender, String[] cmdstring) {
        EntityPlayer player;
        if (sender instanceof Entity &&
                (player = sender.getEntityWorld().getPlayerEntityByName(sender.getName())) != null) {
            if (cmdstring.length < 2 || cmdstring[1].equalsIgnoreCase("help")) {
                sender.sendMessage(new TextComponentString(
                        cmdstring[0] + " <dimId> - teleports the player to the supplied dimension"));
                sender.sendMessage(new TextComponentString(
                        cmdstring[0] + " station <station ID> - teleports the player to the supplied station"));
                return;
            }
            try {
                int dim;
                ...
                if (cmdstring.length == 2) {
                    ...
                    if (net.minecraftforge.common.DimensionManager.isDimensionRegistered(dim)) {
                        ...
                        player.getServer().getPlayerList().transferPlayerToDimension((EntityPlayerMP) player, dim,
                    ...
                    if (spaceObject != null) {
                        if (player.world.provider.getDimension() != ARConfiguration.getCurrentConfig().spaceDimId)
                            player.getServer().getPlayerList().transferPlayerToDimension((EntityPlayerMP) player, dim,
                                    new TeleporterNoPortal((WorldServer) player.world));
    ...
    private void commandFetch(ICommandSender sender, String[] cmdstring) {
        ...
        if (player == null) {
            ...
            player.getServer().getPlayerList().transferPlayerToDimension((EntityPlayerMP) player,
                    me.world.provider.getDimension(),
                    new TeleporterNoPortal(me.getServer().getWorld(me.world.provider.getDimension())));
    ...
    private void commandPlanetList(ICommandSender sender, String[] cmdstring) {
        ...
        for (int i : DimensionManager.getInstance().getRegisteredDimensions()) {
            sender.sendMessage(new TextComponentString(
                    "DIM" + i + ":  " + DimensionManager.getInstance().getDimensionProperties(i).getName()));
        }
    }
    ...
    private void commandPlanetHelp(ICommandSender sender, String[] cmdstring) {
        ...
        sender.sendMessage(new TextComponentString(
                "planet generate [starId] (moon/gas) [name] [atmosphere randomness] [distance Randomness] [gravity randomness] (atmosphere base) (distance base) (gravity base)"));
        ...
        if (cmdstring.length == 3) {
            try {
                ...
                PacketHandler.sendToAll(
                        new PacketDimInfo(dimId, DimensionManager.getInstance().getDimensionProperties(dimId)));
            } catch (NumberFormatException e) {
            ...
            if (sender.getCommandSenderEntity() != null) {
                if (DimensionManager.getInstance()
                        .isDimensionCreated((dimId = sender.getEntityWorld().provider.getDimension()))) {
                    DimensionManager.getInstance().getDimensionProperties(dimId).resetProperties();
                    PacketHandler.sendToAll(
                            new PacketDimInfo(dimId, DimensionManager.getInstance().getDimensionProperties(dimId)));
                }
        ...
        if (cmdstring.length == 3) {
            ...
            try {
                ...
                if (DimensionManager.getInstance().isDimensionCreated(deletedDimId)) {
                    ...
                    if (net.minecraftforge.common.DimensionManager.getWorld(deletedDimId) == null ||
                            net.minecraftforge.common.DimensionManager.getWorld(deletedDimId).playerEntities
                                    .isEmpty()) {
                        ...
                        for (EntityPlayer player : net.minecraftforge.common.DimensionManager
                                .getWorld(deletedDimId).playerEntities) {
            ...
            sender.sendMessage(
                    new TextComponentString(cmdstring[0] + " " + cmdstring[1] + " " + cmdstring[2] + " <dimid>"));
        }
    }
    ...
    private void commandPlanetGenerate(ICommandSender sender, String[] cmdstring) {
        ...
        if (cmdstring.length > 2 + gasOffset) {
            if (cmdstring[2 + gasOffset].equalsIgnoreCase("moon")) {
                ...
                if (!DimensionManager.getInstance().isDimensionCreated(starId)) {
                    ...
                    sender.sendMessage(new TextComponentString(cmdstring[0] + " " + cmdstring[1] +
                            "[planetId] [moon] [gas] <name> <atmosphereRandomness> <distanceRandomness> <gravityRandomness>"));
                    return;
                }
            } else if (DimensionManager.getInstance().getStar(starId) == null) {
                sender.sendMessage(new TextComponentString("Invalid star ID"));
                sender.sendMessage(new TextComponentString(cmdstring[0] + " " + cmdstring[1] +
                        "[starId] [gas] <name> <atmosphereRandomness> <distanceRandomness> <gravityRandomness>"));
        ...
        try {
            ...
            if (cmdstring.length == 6 + gasOffset) {
                ...
                if (!gassy)
                    properties = DimensionManager.getInstance().generateRandom(starId, cmdstring[2 + gasOffset],
                            Integer.parseInt(cmdstring[3 + gasOffset]), Integer.parseInt(cmdstring[4 + gasOffset]),
                            Integer.parseInt(cmdstring[5 + gasOffset]));
                else
                    properties = DimensionManager.getInstance().generateRandomGasGiant(starId, cmdstring[2 + gasOffset],
                            Integer.parseInt(cmdstring[3 + gasOffset]), Integer.parseInt(cmdstring[4 + gasOffset]),
                            Integer.parseInt(cmdstring[5 + gasOffset]), 1, 1, 1);

                if (properties == null)
                    sender.sendMessage(
                            new TextComponentString("Dimension: " + cmdstring[2 + gasOffset] + " failed to generate!"));
                else
                    sender.sendMessage(
                            new TextComponentString("Dimension: " + cmdstring[2 + gasOffset] + " generated!"));
                ...
                if (!gassy)
                    properties = DimensionManager.getInstance().generateRandom(starId, cmdstring[2 + gasOffset],
                            Integer.parseInt(cmdstring[3 + gasOffset]), Integer.parseInt(cmdstring[4 + gasOffset]),
                            Integer.parseInt(cmdstring[5 + gasOffset]), Integer.parseInt(cmdstring[6 + gasOffset]),
                            Integer.parseInt(cmdstring[7 + gasOffset]), Integer.parseInt(cmdstring[8 + gasOffset]));
                else
                    properties = DimensionManager.getInstance().generateRandomGasGiant(starId, cmdstring[2 + gasOffset],
                            Integer.parseInt(cmdstring[3 + gasOffset]), Integer.parseInt(cmdstring[4 + gasOffset]),
                            Integer.parseInt(cmdstring[5 + gasOffset]), Integer.parseInt(cmdstring[6 + gasOffset]),
                ...
                sender.sendMessage(new TextComponentString(cmdstring[0] + " " + cmdstring[1] +
                        " [starId] [moon] [gas] <name> <atmosphereRandomness> <distanceRandomness> <gravityRandomness>"));
                sender.sendMessage(new TextComponentString(""));
                sender.sendMessage(new TextComponentString(cmdstring[0] + " " + cmdstring[1] +
                        " [starId] [moon] [gas] <name> <atmosphere base value> <distance base value> <gravity base value> <atmosphereRandomness> <distanceRandomness> <gravityRandomness>"));
            }
        } catch (NumberFormatException e) {
            sender.sendMessage(new TextComponentString(cmdstring[0] + " " + cmdstring[1] +
                    " [starId] [moon] [gas] <name> <atmosphereRandomness> <distanceRandomness> <gravityRandomness>"));
    ...
    private void commandPlanetSet(ICommandSender sender, String[] cmdstring) {
        ...
        if (cmdstring.length > 3) {
            try {
                ...
                // XXX: Ugh... ordering sucks, what properties does the dimension class have if you don't know what dim
                // it is yet?
        ...
        try {
            if (cmdstring[2 + commandOffset].equalsIgnoreCase("atmosphereDensity")) {
                ...
                sender.sendMessage(new TextComponentString("Setting " + cmdstring[2 + commandOffset] +
                        " for dimension " + dimId + " to " + cmdstring[3 + commandOffset]));
            } else {
                ...
                if (field.getType().isArray()) {
                    ...
                    if (Float.TYPE == field.getType().getComponentType()) {
                        ...
                        if (cmdstring.length - 3 - commandOffset == var.length) {
                            ...
                            sender.sendMessage(new TextComponentString("Setting " + cmdstring[2 + commandOffset] +
                                    " for dimension " + dimId + " to " + outString));
                        }
                    }
            ...
            sender.sendMessage(
                    new TextComponentString("Invalid argument for parameter " + cmdstring[2 + commandOffset]));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
            ...
            if (star == null)
                ...
                if (cmdstring[2].equalsIgnoreCase("temp")) {
                    ...
                    for (IDimensionProperties planets : star.getPlanets()) {
                        sender.sendMessage(
                                new TextComponentString("ID: " + planets.getId() + " : " + planets.getName()));
                    }
    ...
    private void commandBiomeDump(ICommandSender sender, String[] cmdstring) {
        ...
        try {
            ...
            for (ResourceLocation resource : Biome.REGISTRY.getKeys()) {
                writer.append(String.valueOf(Biome.getIdForBiome(Biome.REGISTRY.getObject(resource)))).append("\t")
                        .append(resource.toString()).append("\n");
            }

            writer.close();
            sender.sendMessage(
                    new TextComponentString("The file \"BiomeDump.txt\" has been written to the current directory"));
        } catch (Exception e) {
    ...
    private void commandStarGenerate(ICommandSender sender, String[] cmdstring) {
        try {
            ...
            if (star.getId() != -1) {
                ...
                sender.sendMessage(new TextComponentString(
                        "Why can't I hold all these stars! (either you have an insane number of stars or something really broke!)"));
        ...
        if (cmdstring.length > 1) {
            if (cmdstring[1].equalsIgnoreCase("list")) {
                for (StellarBody star : DimensionManager.getInstance().getStars())
                    sender.sendMessage(new TextComponentString(String.format("Star ID: %d   Name: %s  Num Planets: %d",
    ...
    public List<String> getTabCompletions(MinecraftServer server,
        ...
        } else if ((string[1].equalsIgnoreCase("get") || string[1].equalsIgnoreCase("set")) &&
                string[0].equalsIgnoreCase("planet") && string.length == 3) {
