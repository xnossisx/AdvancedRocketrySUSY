...
public class MapGenCraterHuge extends MapGenBase {
    ...
    private static boolean isCraterIgnoredBlock(Block block) {
        return block instanceof BlockLiquid || block instanceof IFluidBlock || block == Blocks.AIR ||
                block == Blocks.ICE;
    }
    ...
    @Override
    protected void recursiveGenerate(World world, int chunkX, int chunkZ, int p_180701_4_, int p_180701_5_,
        ...
        if (rand.nextInt(chancePerChunk) == Math.abs(chunkX) % chancePerChunk &&
                ...
                shouldCraterSpawn(DimensionManager.getInstance().getDimensionProperties(world.provider.getDimension()),
                        world.getBiome(new BlockPos(chunkX * 16, 0, chunkZ * 16)))) {

            // Random coefficients for the sin functions
            int[] sinCoefficients = { rand.nextInt(10) + 1, rand.nextInt(10) + 1, rand.nextInt(10) + 1,
            ...
            for (int x = 15; x >= 0; x--) {
                for (int z = 15; z >= 0; z--) {
                    for (int y = 254; y >= 0; y--) {
                        if (chunkPrimerIn.getBlockState(x, y, z).getBlock() instanceof BlockLiquid ||
                                chunkPrimerIn.getBlockState(x, y, z).getBlock() instanceof IFluidBlock) {
                            if (y > fluidMaxY) {
            ...
            for (int x = 15; x >= 0; x--) {
                for (int z = 15; z >= 0; z--) {
                    for (int y = 254; y >= 0; y--) {
                        if (y <= fluidMaxY && fillBlock.getBlock() != Blocks.AIR &&
                                chunkPrimerIn.getBlockState(x, y, z).getBlock() == Blocks.AIR) {
                        ...
                        if (!isCraterIgnoredBlock(chunkPrimerIn.getBlockState(x, y, z).getBlock())) {
                            ...
                            int radius = getRadius(baseRadius, (xCoord * 16) + x, (zCoord * 16) + z, numBulges,
                                    sinCoefficients);

                            // Standard inversePartialSquareRadius & blockRadius stuff
                            int distancesSquared = ((xCoord * 16) + x) * ((xCoord * 16) + x) +
                                    ((zCoord * 16) + z) * ((zCoord * 16) + z);
                            int blockRadius = (int) Math.sqrt(distancesSquared);
                            int inversePartialSquareRadius = (int) ((radius * radius - distancesSquared) /
                                    (baseRadius * ((baseRadius > 256) ? 4 : (baseRadius > 128) ? 3 : 2.25)));
                            ...
                            for (int dist = 0; dist < inversePartialSquareRadius; dist++) {
                                if (y - dist > 2) {
                                    chunkPrimerIn.setBlockState(x, y - Math.min(27, dist), z,
                                            (y - dist <= fluidMaxY) ? fillBlock : Blocks.AIR.getDefaultState());
                                }
                            }
                            ...
                            if (inverseRadius <= radius / 4 && inverseRadius > -3 * radius) {
                                // The graph of this function and the old one can be found here
                                // https://www.desmos.com/calculator/x02rgy2wlf
                                for (int dist = -1; dist < 9 * ridgeSize *
                                        ((1 - inverseRadius) /
                                                (0.8 * radius + (inverseRadius - 1) * (inverseRadius - 1))) -
                                        1.06; dist++) {
                                    // Place the bank thrown up by the impact, and have some of the farthest be
                                    // dispersed
                                    if (y + dist < 255 && inverseRadius > -0.75 * radius)
                                        chunkPrimerIn.setBlockState(x, y + dist, z,
                                                this.getBlockToPlace(world, chunkX, chunkZ, ores));
                                    else if (y + dist < 255 && inverseRadius >= -0.875 * radius &&
                                            !(rand.nextInt(inverseRadius + (int) (radius * 0.875) + 1) == 0))
                                        chunkPrimerIn.setBlockState(x, y + dist, z,
                                                this.getBlockToPlace(world, chunkX, chunkZ, ores));
                                    else if (y + dist < 255 && inverseRadius < -0.875 * radius &&
                                            rand.nextInt(Math.abs(inverseRadius + (int) (radius * 0.875)) + 1) == 0)
                                        chunkPrimerIn.setBlockState(x, y + dist, z,
                                                this.getBlockToPlace(world, chunkX, chunkZ, ores));
                                    ...
                                    if (rand.nextInt(Math.abs(inverseRadius) + 1) == 0) {
                                        if (inverseRadius < -0.375 * radius && inverseRadius > -1.5 * radius)
                                            chunkPrimerIn.setBlockState(x, y + dist + 1, z,
                                                    this.getBlockToPlace(world, chunkX, chunkZ, ores));
                                        else if (inverseRadius < -1.5 * radius)
                                            chunkPrimerIn.setBlockState(x, y + dist + 1 + rand.nextInt(2), z,
                                                    this.getBlockToPlace(world, chunkX, chunkZ, ores));
                                    }
                                }
                            }
                            ...
                            if (inversePartialSquareRadius >= 0) {
                                ...
                                chunkPrimerIn.setBlockState(x, y - Math.min(28, inversePartialSquareRadius), z,
                                        this.getBlockToPlace(world, chunkX, chunkZ, ores));
                                chunkPrimerIn.setBlockState(x, y - 1 - Math.min(28, inversePartialSquareRadius), z,
                                        this.getBlockToPlace(world, chunkX, chunkZ, ores));
                            }
                            ...
                            if (blockRadius < 0.25 * radius && spire) {
                                for (int dist = 0; dist <
                                        Math.pow(Math.abs(-(radius / 16.0) + blockRadius / 4.0), 1.25); dist++) {
                                    chunkPrimerIn.setBlockState(x, y + Math.min(dist, 16) - 27, z,
                                            this.getBlockToPlaceRich(world, chunkX, chunkZ, ores));
                                }
