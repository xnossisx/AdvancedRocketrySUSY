...
public class MapGenGeode extends MapGenBase {
    ...
    private List<BlockMeta> ores; // = {new BlockMeta(Blocks.IRON_ORE), new BlockMeta(Blocks.GOLD_ORE), new
                                  // BlockMeta(Blocks.REDSTONE_ORE), new BlockMeta(Blocks.LAPIS_ORE)};
    ...
    private static boolean canGeodeGenerate(World world, int x, int z) {
        return !BiomeDictionary.hasType(world.getBiome(new BlockPos(x, 0, z)), BiomeDictionary.Type.OCEAN) &&
                !BiomeDictionary.hasType(world.getBiome(new BlockPos(x, 0, z)), BiomeDictionary.Type.RIVER) &&
                !BiomeDictionary.hasType(world.getBiome(new BlockPos(x, 0, z)), BiomeDictionary.Type.BEACH);
    }
    ...
    protected void recursiveGenerate(World world, int chunkX, int chunkZ, int p_180701_4_, int p_180701_5_,
        ...
        ores.addAll(
                props.geodeOres.stream()
                        ...
                        .map(itemStack -> new BlockMeta(Block.getBlockFromItem(itemStack.getItem()),
        ...
        if ((rand.nextInt(chancePerChunk) == Math.abs(chunkX) % chancePerChunk ||
                ...
                canGeodeGenerate(world, chunkX * 16, chunkZ * 16)) {

            int radius = rand.nextInt(ARConfiguration.getCurrentConfig().geodeVariation) +
                    ARConfiguration.getCurrentConfig().geodeBaseSize -
                    (ARConfiguration.getCurrentConfig().geodeVariation / 2); // 24; 24 -> 48
            ...
            for (int x = 15; x >= 0; x--) {
                for (int z = 15; z >= 0; z--) {
                    ...
                    int count = (depth -
                            (((xCoord * 16) + x) * ((xCoord * 16) + x) + ((zCoord * 16) + z) * ((zCoord * 16) + z))) /
                            (radius * 2);
                    ...
                    if (count >= 0) {
                        ...
                        chunkPrimerIn.setBlockState(x, avgY - count, z,
                                AdvancedRocketryBlocks.blocksGeode.getDefaultState());
                        chunkPrimerIn.setBlockState(x, avgY + count, z,
                                AdvancedRocketryBlocks.blocksGeode.getDefaultState());
                    }
                }
