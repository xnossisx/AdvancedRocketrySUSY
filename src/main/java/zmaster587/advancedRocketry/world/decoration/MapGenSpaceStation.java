...
public class MapGenSpaceStation {
    ...
    public static void generateStation(World world, int blockX, int blockY, int blockZ) {
        ...
        for (EnumFacing dir : EnumFacing.HORIZONTALS) {
            world.setBlockState(pos.offset(dir),
                    Blocks.PISTON.getDefaultState().withProperty(BlockPistonBase.FACING, dir), 6);
    ...
    private static void generateArm(World world, BlockPos pos, EnumFacing direction) {
        for (int i = 0; i < 8; i++) {
            world.setBlockState(pos.offset(direction, i + 2), AdvancedRocketryBlocks.blockConcrete.getDefaultState(),
                    6);
        }
    }
}
