...
public class PlanetaryTravelHelper {
     ...
     */
    public static boolean isTravelBetweenBodiesWithinPlanetarySystem(int currentDimensionID,
                                                                     int destinationDimensionID) {
        boolean isPlanetMoonSystem = false;
        IDimensionProperties launchworldProperties = DimensionManager.getInstance()
                .getDimensionProperties(currentDimensionID);
     ...
     * @return integer for the number of extra blocks the rocket will have to burn for to complete its injection burn
     *         when orbit height has been reached
     */
    public static int getTransbodyInjectionBurn(int currentDimensionID, int destinationDimensionID,
        ...
        // This is probably one of the worst ways to do this and I don't really care about realism, just tapering
        // results.... if this turns out to be realistic well then, that's nice.
        ...
        // This grabs the body distance multipier, then takes the square root of it, or if warp multiplies by the config
        // option for that
        return (isTravelBetweenBodiesWithinPlanetarySystem(currentDimensionID, destinationDimensionID)) ?
                (int) (baseInjectionHeight * Math.pow(
                        getBodyDistanceMultiplier(currentDimensionID, destinationDimensionID, toAsteroids), 0.5d)) :
                (int) ARConfiguration.getCurrentConfig().warpTBIBurnMult * baseInjectionHeight;
    }
    ...
    public static double getBodyDistanceMultiplier(int currentDimensionID, int destinationDimensionID,
        ...
        IDimensionProperties destinationProperties = DimensionManager.getInstance()
                .getDimensionProperties(destinationDimensionID);
        if (destinationProperties.isMoon()) {
            ...
            for (int moonDimID : destinationProperties.getChildPlanets()) {
                if (currentDimensionID == moonDimID) {
                    bodyDistanceMultiplier = DimensionManager.getInstance().getDimensionProperties(moonDimID)
                            .getOrbitalDist() / 100d;
                }
            }
        }
    ...
    public static boolean isTravelAnywhereInPlanetarySystem(int currentDimensionID, int destinationDimensionID) {
        return isTravelWithinOrbit(currentDimensionID, destinationDimensionID) ||
                isTravelBetweenBodiesWithinPlanetarySystem(currentDimensionID, destinationDimensionID);
    }
    ...
    public static boolean isTravelWithinGeostationaryOrbit(SpaceStationObject spaceStation, int planetID) {
        // Returns true if the planet and the dimension (can be any!) are the same parent and if station is 36300 > x >
        // 35500 km
