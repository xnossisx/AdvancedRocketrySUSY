...
public class MobileAABB extends AxisAlignedBB {
    ...
    // Good thing mojang's bounding boxes are offset by one depending on if server or client, otherwise i may not need
    // to do this
     ...
     * 
     * double maxUp = 256;
     * for(int deltaY = (int) Math.floor(aabbIn.minY - this.minY ); deltaY <= Math.ceil(aabbIn.maxY - this.minY );
     * deltaY++) {
     * for(int deltaZ = (int) Math.floor(aabbIn.minZ - this.minZ ); deltaZ < Math.ceil(aabbIn.maxZ - this.minZ );
     * deltaZ++) {
     * if(deltaY >= 0 && deltaZ >= 0 && deltaY < chunk.sizeY && deltaZ < chunk.sizeZ) {
     * double checkVar = chunk.blocks[(int)(aabbIn.maxX - this.minX)][deltaY][deltaZ].getBlockBoundsMinX() +
     * Math.floor(aabbIn.minX - this.minX) + 0.5 + minX - aabbIn.minX;
     * if(maxUp > checkVar)
     * maxUp = checkVar;
     * }
     * }
     ...
     * double checkVar = chunk.blocks[(int)(aabbIn.minX - this.minX)][deltaY][deltaZ].getBlockBoundsMaxX() +
     * Math.floor(aabbIn.minX - this.minX) - 1 + minX - aabbIn.minX;
     * if(maxUp < checkVar)
     * maxUp = checkVar;
     * }
     * }
     ...
     * 
     * double maxUp = 256;
     * for(int deltaX = (int) Math.floor(aabbIn.minX - this.minX ); deltaX <= Math.ceil(aabbIn.maxX - this.minX );
     * deltaX++) {
     * for(int deltaY = (int) Math.floor(aabbIn.minY - this.minY ); deltaY < Math.ceil(aabbIn.maxY - this.minY );
     * deltaY++) {
     * if(deltaX >= 0 && deltaY >= 0 && deltaX < chunk.sizeX && deltaY < chunk.sizeY) {
     * double checkVar = chunk.blocks[deltaX][deltaY][(int)(aabbIn.maxZ - this.minZ)].getBlockBoundsMinZ() +
     * Math.floor(aabbIn.minZ - this.minZ) + 1 + minZ - aabbIn.minZ;
     * if(maxUp > checkVar)
     * maxUp = checkVar;
     * }
     * }
     ...
     * deltaY++) {
     * if(deltaX >= 0 && deltaY >= 0 && deltaX < chunk.sizeX && deltaY < chunk.sizeY) {
     * double checkVar = chunk.blocks[deltaX][deltaY][(int)(aabbIn.minZ - this.minZ)].getBlockBoundsMaxZ() +
     * Math.floor(aabbIn.minZ - this.minZ) - 0.5 + minZ - aabbIn.minZ;
     * if(maxUp < checkVar)
     * maxUp = checkVar;
     * }
     * }
        ...
        if (aabbIn.maxX > this.minX && aabbIn.minX < this.maxX) {
            if (aabbIn.maxZ > this.minZ && aabbIn.minZ < this.maxZ) {
                ...
                if (var2 > 0.0D && aabbIn.maxY <= this.minY) {
                    ...
                    double maxUp = 256;
                    for (int deltaX = (int) Math.floor(aabbIn.minX - this.minX); deltaX <=
                            Math.ceil(aabbIn.maxX - this.minX); deltaX++) {
                        for (int deltaZ = (int) Math.floor(aabbIn.minZ - this.minZ); deltaZ <
                                Math.ceil(aabbIn.maxZ - this.minZ); deltaZ++) {
                            if (deltaX >= 0 && deltaZ >= 0 && deltaX < chunk.sizeX && deltaZ < chunk.sizeZ) {

                                // Screw 1.10.2
                                int offset = isRemote ? 0 : 1;
                                AxisAlignedBB aabb = chunk.getblocks()[deltaX][(int) (aabbIn.maxY - this.minY) -
                                        1][deltaZ].getDefaultState().getCollisionBoundingBox(this.chunk.world,
                                                new BlockPos(deltaX, (int) (aabbIn.maxY - this.minY), deltaZ));

                                if (aabb != null) {
                                    double checkVar = aabb.minY + Math.floor(aabbIn.minY - this.minY) + minY -
                                            aabbIn.minY - 0.05 + offset;
                                    if (maxUp > checkVar)
                                        maxUp = checkVar;
                                }
                            }
                ...
                if (var2 < 0.0D && aabbIn.minY >= this.maxY) {
                    ...
                    for (int deltaX = (int) Math.floor(aabbIn.minX - this.minX); deltaX <=
                        ...
                        for (int deltaZ = (int) Math.floor(aabbIn.minZ - this.minZ); deltaZ <
                                Math.ceil(aabbIn.maxZ - this.minZ); deltaZ++) {
                            if (deltaX >= 0 && deltaZ >= 0 && deltaX < chunk.sizeX && deltaZ < chunk.sizeZ &&
                                    (int) (aabbIn.minY - this.minY) < chunk.sizeY) {

                                // Screw 1.10.2
                                int offset = isRemote ? 0 : 1;
                                AxisAlignedBB aabb = chunk
                                        .getblocks()[deltaX][(int) (aabbIn.minY - this.minY - offset)][deltaZ]
                                                .getDefaultState()
                                                .getCollisionBoundingBox(this.chunk.world, new BlockPos(deltaX,
                                                        (int) (aabbIn.minY - this.minY - offset), deltaZ));

                                if (aabb != null) {
                                    float offset2 = 1;
                                    double checkVar = aabb.maxY + Math.floor(aabbIn.minY - this.minY) + minY -
                                            aabbIn.minY - offset2 - offset;
                                    if (maxUp < checkVar)
                                        maxUp = checkVar;
                                }
                            }
        ...
        for (int x = 0; x < chunk.getblocks().length; x++) {
            for (int y = 0; y < chunk.getblocks()[0].length; y++) {
                for (int z = 0; z < chunk.getblocks()[0][0].length; z++) {
                    ...
                    AxisAlignedBB bb = block.getDefaultState().getCollisionBoundingBox(chunk.world,
                            new BlockPos(x, y, z));
