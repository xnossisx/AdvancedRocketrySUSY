...
public class ItemUpgrade extends ItemIngredient implements IArmorComponent {
    ...
    public ItemUpgrade(int num) {
        ...
        walkSpeed = ReflectionHelper.findField(net.minecraft.entity.player.PlayerCapabilities.class, "walkSpeed",
                "field_75097_g");
    ...
    public void onTick(World world, EntityPlayer player, @Nonnull ItemStack armorStack,
        ...
        if (componentStack.getItemDamage() == legUpgradeDamage) {
            if (player.isSprinting()) {
                ...
                for (int i = 0; i < modules.getSizeInventory(); i++) {
                    ...
                    if (!stackInSlot.isEmpty() && stackInSlot.getItem() == this &&
                            stackInSlot.getItemDamage() == legUpgradeDamage) {
                ...
                // ReflectionHelper.setPrivateValue(net.minecraft.entity.player.PlayerCapabilities.class,
                // player.capabilities, (itemCount+1)*0.1f, "walkSpeed", "field_75097_g");
            } else
                try {
            ...
            // ReflectionHelper.setPrivateValue(net.minecraft.entity.player.PlayerCapabilities.class,
            // player.capabilities, 0.1f,"walkSpeed", "field_75097_g");
        } else if (componentStack.getItemDamage() == bootsUpgradeDamage &&
                (!ARConfiguration.getCurrentConfig().lowGravityBoots || DimensionManager.getInstance()
                        .getDimensionProperties(world.provider.getDimension()).getGravitationalMultiplier() < 1f))
